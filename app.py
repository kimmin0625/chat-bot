...
2. 단위(m2, m3 등)나 수식을 출력할 때 문자가 중복해서 나오거나, 원시 코드(`TEXT`, `\text{{...}}`)가 화면에 노출되지 않게 해.
3. 모든 단위는 ㎡, ㎥, m², m³ 와 같은 깔끔한 기호를 사용하거나 텍스트로 명확하게 작성해.
4. 🚨 [매우 중요]: 산출기준, 적용 개소, 예외 조건 등을 설명할 때는 문서에 적힌 아주 작은 세부 사항(예: "적용 개소는 코어개소 적용" 등)도 절대 1글자도 누락하지 말고 빠짐없이 100% 모두 기재해. 문서 내용을 자의적으로 생략하지 마라.
